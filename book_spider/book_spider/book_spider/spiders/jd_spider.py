...
class jd_spider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in l[:1]:
            ...
            for index in range(1,200):
                if is_over:
                    break
                url = i[0]+r"&page="+str(index)+r"&sort=sort_totalsales15_desc&trans=1&JL=4_2_0#J_main"
    ...
    def detail_page(self, response):
        ...
        yield scrapy.Request(url=desc_url, callback=self.get_desc, meta={"item":one}, errback=self.error_handler)
    ...
    def error_handler(self, response):
        print "4040404040404040404040404040404"
